...
from transformers import T5Tokenizer, T5EncoderModel, CLIPTokenizer, CLIPTextModel, CLIPConfig
...
class FrozenCLIPEmbedder(AbstractEncoder):
    ...
    def __init__(self, version="./models/openai--clip-vit-large-patch14", device="cpu", max_length=77):  # clip-vit-base-patch32
        super().__init__()
        print("Started frozen clip class")
...
if __name__ == "__main__":
    ...
    model = FrozenT5Embedder(version="google/t5-v1_1-xl").cpu()
    count_params(model, True)
    z = model(sentences)
    print(z.shape)

    model = FrozenCLIPEmbedder().cpu()
    count_params(model, True)
    z = model(sentences)
    print(z.shape)

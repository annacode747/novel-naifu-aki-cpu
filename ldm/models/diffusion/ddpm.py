...
class DDPM(nn.Module):
    ...
    def p_losses(self, x_start, t, noise=None):
        ...
        if self.tlm is not None:
            with torch.no_grad():
                weights = self.tlm.get_item_weights(loss)
            loss = loss * weights
            del weights
    ...
    def training_step(self, batch, batch_idx, log=True, tlm=None):
...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        if self.tlm is not None:
            with torch.no_grad():
                weights = self.tlm.get_item_weights(loss_simple)
            loss_simple = loss_simple * weights
...
class DiffusionWrapper(nn.Module):
    ...
    def forward(self, x, t, c_crossattn):
